...
function getRandomInterval() {
  return (
    Math.floor(
      Math.random() * (maxObstacleInterval - minObstacleInterval + 1)
    ) + minObstacleInterval
  );
}
...
function gameLoop() {
  ...
  const nextThreshold =
    Math.floor(lastSpeedIncreaseScore / scoreThreshold + 1) * scoreThreshold;
  ...
  if (score >= nextThreshold) {
    ...
    console.log(
      `Score reached ${nextThreshold.toFixed(
        0
      )}. Speed increased by ${nextSpeedIncrement.toFixed(
        2
      )} to ${obstacleSpeed.toFixed(2)}`
    );
...
// --- Sound Effects ---
const startSound = new Audio("sound/game-start-6104.mp3");
...
startScreenImage.src = "backround/start-screen.png";

function drawInitialScreen() {
  // Wait for ALL images including start screen
  if (!checkAllImagesLoaded() || !startScreenImage.complete) {
    ...
    let totalImages = 8; // Including start screen image
    ctx.fillText(
...
bgImage1.onload = () => {
  console.log("BG 1 loaded.");
  isBgImage1Loaded = true;
  if (checkAllImagesLoaded()) drawInitialScreen();
};
bgImage1.onerror = () => console.error("Failed to load background image 1!");
bgImage2.onload = () => {
  console.log("BG 2 loaded.");
  isBgImage2Loaded = true;
  if (checkAllImagesLoaded()) drawInitialScreen();
};
bgImage2.onerror = () => console.error("Failed to load background image 2!");
bgImage3.onload = () => {
  console.log("BG 3 loaded.");
  isBgImage3Loaded = true;
  if (checkAllImagesLoaded()) drawInitialScreen();
};
bgImage3.onerror = () => console.error("Failed to load background image 3!");
bgImage4.onload = () => {
  console.log("BG 4 loaded.");
  isBgImage4Loaded = true;
  if (checkAllImagesLoaded()) drawInitialScreen();
};
bgImage4.onerror = () => console.error("Failed to load background image 4!");
bgImage5.onload = () => {
  console.log("BG 5 loaded.");
  isBgImage5Loaded = true;
  if (checkAllImagesLoaded()) drawInitialScreen();
};
